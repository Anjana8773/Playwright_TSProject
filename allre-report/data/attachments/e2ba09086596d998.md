...
- Samsung galaxy s7
+
+             Samsung galaxy s7800Delete

Call log:
  - Expect "toHaveText" with timeout 5000ms
  - waiting for locator('#tbodyid')
    5 × locator resolved to <tbody id="tbodyid">↵            </tbody>
      - unexpected value "
            "
    9 × locator resolved to <tbody id="tbodyid">…</tbody>
      - unexpected value "
            Samsung galaxy s7800Delete"
...
> 34 |      await expect(page.locator('#tbodyid')).toHaveText('Samsung galaxy s7')
     |                                             ^ Error: expect(locator).toHaveText(expected) failed
  35 | 
  36 | 
  37 | 
