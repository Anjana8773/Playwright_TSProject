...
```
Error: locator.textContent: Error: strict mode violation: locator('#tbodyid').first().locator('.hrefch') resolved to 9 elements:
...
Call log:
  - waiting for locator('#tbodyid').first().locator('.hrefch')
...
```ts
  ...
  126 |      const products = page.locator("#tbodyid")
  127 |      const count = await products.count();
  128 | 
  129 |     for (let i = 0; i < count; ++i) {
> 130 |         if (await products.nth(i).locator(".hrefch").textContent() === pdt_name) {
      |                                                      ^ Error: locator.textContent: Error: strict mode violation: locator('#tbodyid').first().locator('.hrefch') resolved to 9 elements:
  131 |          //add to cart
  132 |          await products.nth(i).locator(".hrefch",{hasText: pdt_name}).click();
  133 |          break;
  134 |       }
  135 | }
  136 |  
  137 |   
  138 |      await addtocartpage.clickAddtoCartBtn()
  139 |      page.on('dialog',async dialog=>{
  140 |          expect(dialog.message()).toBe('Product added.')
  141 |         await dialog.accept()
  142 |      })
  143 | 
  144 | })
  145 | 
  146 | test('TC#8 Add to cart Item Phone',async({page})=>{    
  147 |      const pomanager=new POManager(page)
  148 |      const loginpage=pomanager.getLogin()
  149 |      const addtocartpage=pomanager.getAddtoCartPage()       
  150 |      const username=signupcredentials.username
  151 |      const password=signupcredentials.password 
  152 | 
  153 | 
  154 |      await loginpage.clickLoginLink()
  155 |      await loginpage.userLoginForm(username,password)
  156 |      await loginpage.clicksubmitbtn()        
  157 |      expect(page.locator('#nameofuser')).toHaveText(`Welcome ${username}`)
  158 | 
  159 |     const cartItemsCount=await page.locator('#tbodyid tr').count()
  160 |     const pdt_name='Samsung galaxy s6'
  161 |      expect(cartItemsCount).toBeGreaterThan(0)
  162 |      for(let i=0;i<cartItemsCount;i++){
  163 |          const productTitle=await page.locator('#tbodyid td').nth(i).textContent()  
  164 | 
  165 |          try{
  166 |           
  167 |            if(productTitle!=null &&productTitle.trim()===pdt_name){
  168 |                break
  169 |            
  170 |           }
  171 |      }
  172 |      catch(error){
  173 |            console.error('item not found', error)      
  174 |      }}
  175 |     
  176 |      await addtocartpage.clickAddtoCartBtn()
  177 |      await addtocartpage.clickCartLink()
  178 |      const confirmorderpage=pomanager.getConfirmOrderPage()
  179 |      await addtocartpage.clickPlaceOrderBtn()
  180 |      const name=getCellData('name')
  181 |      const country=getCellData('country')
  182 |      const city=getCellData('city')
  183 |      const creditcard=getCellData('creditcard')
  184 |      const month=getCellData('month')
  185 |      const year=getCellData('year')     
  186 |      await confirmorderpage.fillOrderForm(name,country,city,creditcard,month,year)
  187 |      await confirmorderpage.clickPurchaseBtn()
  188 |      page.on('dialog',async dialog=>{
  189 |          expect(dialog.message()).toContain('Thank you for your purchase!')
  190 |         await dialog.accept()
  191 |      }    
  192 | )
  193 |          
  194 | 
  195 | })
  196 | 
  197 | test('TC#9 Add to cart Item Monitor',async({page})=>{   
  198 |        const pomanager=new POManager(page)
  199 |      const loginpage=pomanager.getLogin()
  200 |      const addtocartpage=pomanager.getAddtoCartPage()       
  201 |      const user=signupcredentials.username
  202 |      const password=signupcredentials.password
  203 | 
  204 |      
  205 |      await loginpage.clickLoginLink()
  206 |      await loginpage.userLoginForm(user,password)
  207 | 
  208 |      await loginpage.clicksubmitbtn()
  209 | 
  210 |      await expect(page.locator('#nameofuser')).toHaveText(`Welcome ${user}`)
  211 | 
  212 |      const cartItemsCount=await page.locator('#tbodyid tr').count()
  213 |      expect(cartItemsCount).toBeGreaterThan(0)
  214 |      const pdt_name='Sony vaio i7'
  215 |      for(let i=0;i<cartItemsCount;i++){
  216 |          const productTitle=await page.locator('#tbodyid td').nth(i).textContent()  
  217 | 
  218 |          try{
  219 |           
  220 |            if(productTitle!=null &&productTitle.trim()===pdt_name){
  221 |                break
  222 |            
  223 |           }
  224 |      }
  225 |      catch(error){
  226 |            console.error('item not found', error)      
  227 |      }}
  228 |      await addtocartpage.clickAddtoCartBtn()
  229 |      page.on('dialog',async dialog=>{
  230 |          expect(dialog.message()).toBe('Product added.')
```