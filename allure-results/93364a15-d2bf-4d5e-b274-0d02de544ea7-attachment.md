...
- Name: validusercredentials.spec.ts >> TC#3 User Valid username and Valid password
- Location: tests\validusercredentials.spec.ts:11:5
...
```ts
  ...
  9  | 
  10 | 
  11 | test('TC#3 User Valid username and Valid password',async({page})=>{
  12 |      const {username,password}=usercredentials[0]
  13 |      const pomanager=new POManager(page)
  14 |      const loginpage=pomanager.getLogin()
  15 |      
  16 |      await loginpage.clickLoginLink()
  17 |      await loginpage.userLoginForm(username,password)  
  18 |      await loginpage.clicksubmitbtn()   
> 19 |      expect(page.locator('#nameofuser')).toHaveText(`Welcome ${username}`)
     |                                          ^ Error: expect(locator).toHaveText(expected) failed
  20 |       
  21 | 
  22 | })
